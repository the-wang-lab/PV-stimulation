function plotDecodingErr3(decErrMean,decErrMeanStim,decErrMeanShuf,...
    decErrStd,decErrStdStim,decErrStdShuf,time,pStim,pShuf,fileName,pathAnal,ylimit,yl,tit) 
    ...
    plot_areaerrorbarXYZ1(decErrMean, decErrMeanStim, decErrMeanShuf,...
        decErrStd, decErrStdStim, decErrStdShuf, options);
    
    hold on;
    minX = min(decErrMean-decErrStd);
    minY = min(decErrMeanStim-decErrStdStim);
    minZ = min(decErrMeanShuf-decErrStdShuf);
    
    maxX = max(decErrMean+decErrStd);
    maxY = max(decErrMeanStim+decErrStdStim);
    maxZ = max(decErrMeanShuf+decErrStdShuf);
