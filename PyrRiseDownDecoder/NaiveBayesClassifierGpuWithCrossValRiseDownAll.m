function NaiveBayesClassifierGpuWithCrossValRiseDownAll(paths,filenames,mazeSess,upDown,task,onlyRun)
% use a subpopulation of neurons to predit time or distance
    ...
    if(upDown == 1) % PyrUp
        ...
        if(task == 1)
            pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalDecoderWithCrossVal\PyrRiseNoCue\'];
        elseif(task == 2)
    ...
    elseif(upDown == 3) % PyrOther
        ...
        if(task == 1)
            pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalDecoderWithCrossVal\PyrRiseDownNoCue\'];
        elseif(task == 2)
            pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalDecoderWithCrossVal\PyrRiseDownAL\'];
        elseif(task == 3)
            pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalDecoderWithCrossVal\PyrRiseDownPL\'];
        end
    end
    ...
    paramC.timeBin = 0.005; %sec
    std0 = 0.1/paramC.timeBin; %0.2
    paramC.timeBin1 = 0.25; %sec
