function modPyr1 = accumPyr3NoStim2ndStimCtrl(paths,filenames,mazeSess,autoCorrPyrAll,...
    ...
    for i = 1:numRec
        ...
        for n = 1:length(pulseMeth)
            ...
            stimInterval = diff(trialNoStim{n});
            interval2 = sum(stimInterval == 2);
            interval3 = sum(stimInterval == 3);
            if(pulseMeth(n) == 4)
                if(interval2>interval3)
                    trialNoStimCtrl = trialNoStim{n}-1;
                else
                    trialNoStimCtrl = trialNoStim{n}-2;
                end
            else
                trialNoStimCtrl = trialNoStim{n}-1;
            end
            trialNoStimTmp = ismember(trialNoStimCtrl,trialNoStim{n});
            ...
            for m = 1:length(indNeu1)
                ...
                filteredSpikeArrayTmp = filteredSpikeArrayRunOnSet{indNeu1(m)}(trialNoStimCtrl,:);
            ...
            for m = 1:length(indNeu1)
                ...
                corrArr = spikeCorrTRun{indNeu1(m)}(trialNoStim{n},modPyr1SigAL.trialNoCtrlGood{i});
                corrArr = corrArr(:);        
