function CompPyrIntInitPeakNoCueVsALSigNoFNeu(mode,pshuffle)
...
% mode = 3, No cue vs AL first 10 recordings
    ...
    if(mode == 1)
        pathAnalAL = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakSigNoFNeuALPL\' num2str(pshuffle) '\'];
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakCmpNoCueSigALPLNoFNeu\' num2str(pshuffle) '\'];
    elseif(mode == 2)
        pathAnalAL = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakSigNoFNeuAL\' num2str(pshuffle) '\'];
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakCmpSigNoCueALNoFNeu\' num2str(pshuffle) '\'];
    else
        pathAnalAL = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakSigNoFNeuAL10\' num2str(pshuffle) '\'];
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakCmpSigNoCueAL10NoFNeu\' num2str(pshuffle) '\'];
    end
    pathAnalNoCue = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakSigNoFNeuNoCue\' num2str(pshuffle) '\'];

    if(exist(pathAnal) == 0)
        mkdir(pathAnal);
    end    

    if(exist([pathAnalAL 'initPeakPyrIntAllRecSigNoFNeu.mat']))
        load([pathAnalAL 'initPeakPyrIntAllRecSigNoFNeu.mat']);
    ...
    if(exist([pathAnalNoCue 'initPeakPyrIntAllRecSigNoFNeu.mat']))
        load([pathAnalNoCue 'initPeakPyrIntAllRecSigNoFNeu.mat']);
    ...
    save([pathAnal 'initPeakPyrIntAllRecNoCueVsALNoFNeu.mat'],'FRProfileMeanPyrStat1',...
