function PyrBehALNoCue(onlyRun)
...
% plot aligned to cue and over distance (onlyRun should be set to 0)
    ...
    pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalBehALNoCue\'];
    
    if(exist([pathAnal 'behNoCueVsAL_AlignedtoCueOverDist.mat']))
        load([pathAnal 'behNoCueVsAL_AlignedtoCueOverDist.mat']);
    end
   
    if(exist('behMeanNoCue') == 0)
        [behNoCue,behMeanNoCue] = accumBehCue(listRecordingsNoCuePath,...
                listRecordingsNoCueFileName,mazeSessionNoCue,1,onlyRun);

        [behAL,behMeanAL] = accumBehCue(listRecordingsActiveLickPath(1:10,:),...
        ...
        save([pathAnal 'behNoCueVsAL_AlignedtoCueOverDist.mat'],...
...
function [beh,behMean] = accumBehCue(paths,filenames,mazeSess,task,onlyRun)
    randTrNo = 15;
    ...
    for i = 1:numRec
        ...
        if(~isempty(trialNoNonStim) & length(trialNoNonStim) > paramF.minNumTr)
            if(isempty(beh.meanLickPerRec))
                beh.meanLickPerRec = lickOverDistCtrl.meanCueStim;
                beh.semLickPerRec = lickOverDistCtrl.SEMCueStim;

                beh.meanSpeedPerRec = speedOverDistCtrl.meanCueNonStim;
                beh.semSpeedPerRec = speedOverDistCtrl.SEMCueNonStim;
            else
                beh.meanLickPerRec = [beh.meanLickPerRec; lickOverDistCtrl.meanCueStim];
                beh.semLickPerRec = [beh.semLickPerRec; lickOverDistCtrl.SEMCueStim];

                beh.meanSpeedPerRec = [beh.meanSpeedPerRec; speedOverDistCtrl.meanCueNonStim];
                beh.semSpeedPerRec = [beh.semSpeedPerRec; speedOverDistCtrl.SEMCueNonStim];
            end
        end
        ...
        if(nTr > randTrNo)
            indTrR = randperm(nTr);
            beh.lickTracePerRec{i} = lickOverDist.Cue(trialNoNonStim(indTrR(1:randTrNo)),:);
            beh.speedTracePerRec{i} = speedOverDist.Cue(trialNoNonStim(indTrR(1:randTrNo)),:);
        else
            beh.lickTracePerRec{i} = lickOverDist.Cue(trialNoNonStim,:);
            beh.speedTracePerRec{i} = speedOverDist.Cue(trialNoNonStim,:);
        end
    end
