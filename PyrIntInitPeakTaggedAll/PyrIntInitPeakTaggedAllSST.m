function PyrIntInitPeakTaggedAllSST()
% compare Pyramidal neurons and SST interneurons on their initial peak (including all the recordings where the number of good trials >= minNumTr...
...
% firing rate change and comparing avgProfile. Use PyrIntInitPeak for that (where the neurons with >= minNumTr bad trials are considered)
    ...
    pathAnal = 'Z:\Yingxue\Draft\PV\PyramidalIntInitPeakTaggedAllSST\';
    ...
    if(exist('InitAllTag') == 0)
        ...
        InitAllTag = PyrIntInitPeakByTypeTaggedAllSST(autoCorrIntTag,modInt1AL,minNumTr,minNumTrBad);
    ...
    load([pathAnalTmp 'autoCorrIntAllRec.mat'],'cluSST');
    plotIntNeuRiseDownTaggedAllPVNonPV(pathAnal,modInt1NoCue.timeStepRun,...
            InitAllTag,FRProfileMean,cluSST);
end
