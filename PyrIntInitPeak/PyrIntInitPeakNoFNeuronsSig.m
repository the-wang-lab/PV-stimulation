function PyrIntInitPeakNoFNeuronsSig(taskSel,pshuffle)
% compare Pyramidal neurons and PV interneurons on their initial peak
% only consider neurons without fields
% (pshuffle = 1 -> 99.9%, 2 -> 99%, 3 -> 95%)
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakNoFNeuSig\' num2str(pshuffle) '\'];
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakNoFNeuSigALPL\' num2str(pshuffle) '\'];
    else
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakNoFNeuSigAL\' num2str(pshuffle) '\'];
    end
    if(exist(pathAnal) == 0)
        mkdir(pathAnal);
    end
    if(exist([pathAnal 'initPeakPyrIntAllRecNoFNeuSig.mat']))
        load([pathAnal 'initPeakPyrIntAllRecNoFNeuSig.mat']);
    end
    ...
    if(exist('InitAllPyr') == 0)
        ...
        save([pathAnal 'initPeakPyrIntAllRecNoFNeuSig.mat'],'InitAllPyr','PyrRise','PyrDown','PyrOther',...
