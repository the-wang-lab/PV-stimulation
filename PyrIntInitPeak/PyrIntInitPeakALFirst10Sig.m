function PyrIntInitPeakALFirst10Sig(taskSel,pshuffle)
...
% (pshuffle = 1 -> 99.9%, 2 -> 99%, 3 -> 95%)
    ...
    elseif(taskSel == 4)
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakSigNoCue\' num2str(pshuffle) '\'];
    elseif(taskSel == 5)
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalIntInitPeakSigAL10\' num2str(pshuffle) '\'];
    end
    if(exist(pathAnal) == 0)
        mkdir(pathAnal);
    end
    ...
    if(exist('InitAllPyr') == 0)
        ...
        save([pathAnal 'initPeakPyrIntAllRecSig.mat'],'InitAllPyr','PyrRise','PyrDown','PyrOther',...
