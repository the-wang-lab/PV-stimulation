function PyrIntInitPeakRelRatioNoFNeurons(taskSel)
% compare Pyramidal neurons and PV interneurons on their initial peak
    ...
    if(taskSel == 1) % including all the neurons
        pathAnal = 'Z:\Yingxue\Draft\PV\PyramidalIntInitPeakRelRNoFNeu\';
    elseif(taskSel == 2) % including AL and PL neurons
        pathAnal = 'Z:\Yingxue\Draft\PV\PyramidalIntInitPeakRelRNoFNeuALPL\';
    else
        pathAnal = 'Z:\Yingxue\Draft\PV\PyramidalIntInitPeakRelRNoFNeuAL\';
    end
    if(exist(pathAnal) == 0)
        mkdir(pathAnal);
    end
    if(exist([pathAnal 'initPeakPyrIntRelRAllRecNoFNeu.mat']))
        load([pathAnal 'initPeakPyrIntRelRAllRecNoFNeu.mat']);
    end
...
%     if(exist('InitAll') == 0)
    ...
    %     idx1 = find(ratio0to1BefRunOrd >= 0.5,1,'last');
        ...
        save([pathAnal 'initPeakPyrIntRelRAllRecNoFNeu.mat'],'InitAll','PyrRise','PyrDown','PyrOther',...
