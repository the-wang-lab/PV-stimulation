function PyrIntInitPeakPVStimCtrlSelRec1Excl(methodKMean)
% compare Pyramidal neurons and PV interneurons on their initial peak
% excluding the recordings defined in GlobalConstFq
%% select neuron whose difference in baseline firing rate between ctrl and stim ctrl trials is between 5% to 95% 
%% in the distribution of all the neurons 
    
    pathAnal0 = ['Z:\Yingxue\Draft\PV\PyramidalPVStimALCtrlSel1SelRec\'];
...
%     ind = ~ismember(modPyr1AL.indRec, recExcl);
    ...
    indBL = modPyr1AL.timeStepRun >= -3.0 & modPyr1AL.timeStepRun < -1.0;
    meanBL = mean(modPyr1AL.avgFRProfile(:,indBL),2);
    meanBLStim = mean(modPyr1AL.avgFRProfileStim(:,indBL),2);
    meanBLStimCtrl = mean(modPyr1AL.avgFRProfileStimCtrl(:,indBL),2);
    diffMeanBL = (meanBLStimCtrl - meanBL);
    indStim3Loc120 = find(modPyr1AL.pulseMeth == 3 & modPyr1AL.stimLoc == 120);
    diffMeanBL(indStim3Loc120) = meanBLStim(indStim3Loc120) - meanBL(indStim3Loc120);
    ind = diffMeanBL > prctile(diffMeanBL,5) & diffMeanBL < prctile(diffMeanBL,95);
