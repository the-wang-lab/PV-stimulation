function PyrIntInitPeakPVStimNonStim2ndStimCtrlSelSigChange(methodKMean,p1,pdrift)
% compare Pyramidal neurons and PV interneurons on their initial peak
    
    pDriftStr = num2str(pdrift);
    tmp = strfind(pDriftStr,'.');
    pDriftStr(tmp) = 'p';
    pathAnal0 = ['Z:\Yingxue\Draft\PV\PyramidalStimALNonStim2ndStimCtrlSelSigP' num2str(p1) '-DriftP' pDriftStr '\'];
    ...
    ind = modPyr1AL.pKSMeanFRNonStimVsStimCtrl0to1 >= pdrift; % p value threshold for removing neurons with drift
