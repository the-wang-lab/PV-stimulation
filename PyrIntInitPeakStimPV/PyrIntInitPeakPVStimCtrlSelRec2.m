function PyrIntInitPeakPVStimCtrlSelRec2(methodKMean)
...
%% select neuron that do not have significant difference in baseline firing rate between ctrl and stim ctrl trials
    ...
    pathAnal0 = ['Z:\Yingxue\Draft\PV\PyramidalPVStimALCtrlSel2\'];
...
%     ind = ~ismember(modPyr1AL.indRec, recExcl);
    ...
    indTmp =  modPyr1AL.pulseMeth == 3 & modPyr1AL.stimLoc == 120;
    ind1 = modPyr1AL.pRSMeanFRNonStimVsStimCtrl0to1(indTmp) >= 0.01 & ...
        modPyr1AL.pKSMeanFRNonStimVsStimCtrl0to1(indTmp) >= 0.01;
    ind(indTmp) = ind(indTmp)&ind1;
