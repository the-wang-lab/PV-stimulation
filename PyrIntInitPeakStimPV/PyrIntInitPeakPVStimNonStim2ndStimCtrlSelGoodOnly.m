function PyrIntInitPeakPVStimNonStim2ndStimCtrlSelGoodOnly(methodKMean,stimCtrlCond)
...
% stimCtrlCond = 2, only include sessions where 2 stim ctrl are followed by one stim trial
    ...
    if(stimCtrlCond == 1)
        pathAnal0 = ['Z:\Yingxue\Draft\PV\PyramidalStimALNonStim2ndStimCtrlSelGoodOnly\'];
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalStimALNonStim2ndStimCtrlSigGoodOnly\' num2str(methodKMean) '\'];
    else
        pathAnal0 = ['Z:\Yingxue\Draft\PV\PyramidalStimALNonStim2ndStimCtrlSigGoodOnly2\'];
        pathAnal = ['Z:\Yingxue\Draft\PV\PyramidalStimALNonStim2ndStimCtrlSigGoodOnly2\' num2str(methodKMean) '\'];
    end
    ...
    if(exist([pathAnal0 'initPeakPyrIntAllRecStimSigNoStim2ndStimCtrlSelGoodOnly.mat']))
        load([pathAnal0 'initPeakPyrIntAllRecStimSigNoStim2ndStimCtrlSelGoodOnly.mat']);
    end
    ...
    %     idx1 = find(ratio0to1BefRunOrd >= 0.5,1,'last');
        ...
        save([pathAnal0 'initPeakPyrIntAllRecStimSigNoStim2ndStimCtrlGoodOnly.mat'],'PyrStim1','PyrRise','PyrDown','PyrOther','-v7.3'); 
    end
