function PyrIntInitPeakPVStimNonStim2ndStimCtrlSelSigChangeNoDrift(methodKMean,p1,pdrift)
% compare Pyramidal neurons and PV interneurons on their initial peak
    
    pathAnal0 = ['Z:\Yingxue\Draft\PV\PyramidalStimALNonStim2ndStimCtrlSelSigP' num2str(p1) '-NoDriftRec1\'];
...
%     end
    ...
    if(~isfield(modPyr1AL,'indDriftRec1')) 
        %% detect recordings with significant drift
        recDrift = [];
        pSR = zeros(1,max(modPyr1AL.indRec)); 
        pSR0to1 = zeros(1,max(modPyr1AL.indRec)); 
        pSR3to4 = zeros(1,max(modPyr1AL.indRec)); 
        indFRAll = find(modPyr1AL.timeStepRun >= -1 & modPyr1AL.timeStepRun < 4);
        indFR0to1 = find(modPyr1AL.timeStepRun >= 0.5 & modPyr1AL.timeStepRun < 1.5);
        indFR3to4 = find(modPyr1AL.timeStepRun >= 3 & modPyr1AL.timeStepRun < 4);
        for i = unique(modPyr1AL.indRec)
            ...
            meanStim = mean(meanStim,2);
%             [~,pSR(i)] = ttest(meanCtrl,meanStim);
            pSR(i) = signrank(meanCtrl,meanStim);
            
            meanCtrl = modPyr1AL.avgFRProfile(modPyr1AL.indRec == i,indFR0to1);
            meanCtrl = mean(meanCtrl,2);
            meanStim = modPyr1AL.avgFRProfileStimCtrl(modPyr1AL.indRec == i,indFR0to1);
            meanStim = mean(meanStim,2);
%             [~,pSR0to1(i)] = ttest(meanCtrl,meanStim);
            pSR0to1(i) = signrank(meanCtrl,meanStim);
            
            meanCtrl = modPyr1AL.avgFRProfile(modPyr1AL.indRec == i,indFR3to4);
            meanCtrl = mean(meanCtrl,2);
            meanStim = modPyr1AL.avgFRProfileStimCtrl(modPyr1AL.indRec == i,indFR3to4);
            meanStim = mean(meanStim,2);
%             [~,pSR3to4(i)] = ttest(meanCtrl,meanStim);
            pSR3to4(i) = signrank(meanCtrl,meanStim);
            
            if(pSR(i) < pdrift & pSR0to1(i) < pdrift & pSR3to4(i) < pdrift)
                recDrift = [recDrift i];
            end
        end
        indDriftRec = ~ismember(modPyr1AL.indRec, recDrift);
        modPyr1AL.recDrift1 = recDrift;
        modPyr1AL.indDriftRec1 = indDriftRec;
        modPyr1AL.pSRMeanFRCtrlVsStim = pSR;
        modPyr1AL.pSR0to1MeanFRCtrlVsStim = pSR0to1;
        modPyr1AL.pSR3to4MeanFRCtrlVsStim = pSR3to4;
%         save([pathAnal 'initPeakPyrAllRecSigStimNoStim2ndStimCtrl.mat'],'modPyr1AL'); 
    end
    
    ind = modPyr1AL.indDriftRec1;
