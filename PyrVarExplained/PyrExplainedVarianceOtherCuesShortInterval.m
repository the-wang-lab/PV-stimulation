function PyrExplainedVarianceOtherCuesShortInterval()
...
%% explained by the firing rate from [-1.5 1.5] sec or [-1 1] sec around the cue onset or reward onset  
    ...
    if(exist('varPyrNoCue1') == 0)
        ...
        fullpath = [pathAnal0 'varPyrAllRecCueRewShortInterval.mat'];
...
function varPyr = calVarPyr(indTime,timeStep,indNeu,filteredSpikeArrayOnSet,...
    ...
    indTimeOnset0p5 = timeStep/1250 >= -0.5 & timeStep/1250 <= 0.5; % indices between -1.5 to 1.5 sec around run onset
    indTimeOnset0p3 = timeStep/1250 >= -0.3 & timeStep/1250 <= 0.3; % indices between -1 to 1 sec around run onset
    ...
    parfor n = 1:length(indNeu)
        ...
        perTrRunOnset0p5{n} = var(filteredSpikeArrayOnSet{indNeu(n)}(trialNoNonStimGood,indTimeOnset0p5)'); % between -0.5 to 0.5 sec around run onset
        perTrRunOnset0p3{n} = var(filteredSpikeArrayOnSet{indNeu(n)}(trialNoNonStimGood,indTimeOnset0p3)');
