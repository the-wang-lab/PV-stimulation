function PyrExplainedVarAllRecSigShortInterval(taskSel,pshuffle)
...
%% accumulate over all the recordings
    ...
    if(exist([pathAnal 'expVarPyrAllRecSigShortInterval.mat']))
        load([pathAnal 'expVarPyrAllRecSigShortInterval.mat']);
    end
...
function ExpVarAllPyr = PyrIntInitPeakByType(taskSel,modPyr1NoCue,modPyr1AL,modPyr1PL,...
    ...
    elseif(taskSel == 2)
        if(sum(modPyr1AL.indNeuGood-varPyrAL.indNeu) == 0 && ...
            ...
            ExpVarAllPyr.pFstat0p5 = [varPyrAL.pFsta0pt0p5 varPyrPL.pFstat0p5]; 
