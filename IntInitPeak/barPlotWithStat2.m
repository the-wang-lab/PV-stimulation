function barPlotWithStat2(ind,x,y,data,ylab,tit,rankXY,fn,pathAnal,colorSel,labels,xl)
    ...
    if(max(data(:)) > 0)
        set(gca,'fontSize',12,'YLim',[min(data(:)) 1.1*max(data(:))])
    elseif((max(data(:)) < 0))
        set(gca,'fontSize',12,'YLim',[1.1*min(data(:)) max(data(:))])
    else
    ...
    if(nargin == 12)
       set(gca,'YLim',xl) 
       text(1,1*xl(2),num2str(rankXY,'p = %f'));
    end    
