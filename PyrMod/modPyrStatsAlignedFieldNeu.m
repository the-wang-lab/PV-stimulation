function modPyrStatsFieldF = modPyrStatsAlignedFieldNeu(mod,autoCorrPyrAll)
% statistics for field vs no field neurons for each cluster (after aligning to run onset)
% this function is called by PyrModAllRec.m
    
    modPyrStatsFieldF = [];
    idxC = mod.idxC;
    for i = 1:max(idxC)        
        indCurCField = idxC == i & mod.isNeuWithFieldAligned == 1;
        indCurCNoField = idxC == i & mod.isNeuWithFieldAligned == 0;
        indCurCFieldBurst = idxC == i & mod.isNeuWithFieldAligned == 1 & mod.fractBurst > 0;
        indCurCNoFieldBurst = idxC == i & mod.isNeuWithFieldAligned == 0 & mod.fractBurst > 0;
